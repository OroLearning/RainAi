  ...
  return (
    <div className={`w-screen h-screen flex font-sans bg-[#fcfdfe] dark:bg-darkBg text-slate-900 dark:text-slate-100 overflow-hidden transition-colors duration-0`}>
      ...
      <aside 
        ...
        className={`fixed top-0 left-0 h-full z-[110] flex flex-col bg-white dark:bg-darkCard border-r border-slate-100 dark:border-darkBorder shadow-heavy transition-all duration-500 ease-in-out transform ${isSidebarVisible ? 'w-80 translate-x-0' : 'w-80 -translate-x-full'}`}
      >
        <div className="p-10 pb-8 flex items-center justify-between">
          <div className="flex items-center gap-3 cursor-pointer" onClick={onBack}>
            <div className="w-10 h-10 bg-gradient-to-tr from-blue-700 to-indigo-500 rounded-xl flex items-center justify-center">
              <svg className="w-6 h-6 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                <path d="M6 16.326A7 7 0 1 1 15.71 8h1.79a4.5 4.5 0 0 1 .5 8.973" />
                <path d="m13 11-4 6h6l-4 6" />
              </svg>
            </div>
            <span className="text-2xl font-black uppercase tracking-tight text-slate-900 dark:text-white">RAIN <span className="text-blue-600">Ai</span></span>
          </div>
          <button 
            onClick={() => setIsSidebarPinned(!isSidebarPinned)}
            className={`p-2.5 rounded-xl transition-all duration-300 ${isSidebarPinned ? 'text-blue-600 bg-blue-50 dark:bg-blue-900/20' : 'text-slate-300 hover:text-slate-900 dark:hover:text-white'}`}
          >
        ...
        <nav className="flex-1 px-8 space-y-4">
          <button onClick={() => navigateToTab('workspace')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-sm font-black transition-all ${activeTab === 'workspace' && !showSettings ? 'bg-slate-900 dark:bg-blue-600 text-white shadow-xl' : 'text-slate-400 dark:text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800'}`}>
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg>
            Workspace
          </button>
          <button onClick={() => navigateToTab('history')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-sm font-black transition-all ${activeTab === 'history' && !showSettings ? 'bg-slate-900 dark:bg-blue-600 text-white shadow-xl' : 'text-slate-400 dark:text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800'}`}>
        ...
        <div className="p-8 border-t border-slate-50 dark:border-darkBorder">
           <button onClick={() => { setShowSettings(true); setSettingsView('main'); }} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-sm font-black transition-all ${showSettings ? 'bg-slate-900 dark:bg-blue-600 text-white shadow-xl' : 'text-slate-400 dark:text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800'}`}>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-slate-900/20 dark:bg-black/60 backdrop-blur-md animate-fade-in" onClick={handleCloseSettings}>
          <div className="w-full max-w-2xl bg-white dark:bg-darkCard rounded-[48px] p-12 shadow-heavy border border-slate-100 dark:border-darkBorder animate-fade-in-up" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-12">
               <h2 className="text-4xl font-black uppercase tracking-tight dark:text-white">
            ...
            <div className="space-y-6">
                 ...
                 <>
                   <div className="bg-slate-50 dark:bg-slate-900/50 p-8 rounded-[36px] border border-slate-100 dark:border-darkBorder flex items-center justify-between">
                      ...
                      <button onClick={() => setDarkMode(!darkMode)} className="relative w-24 h-12 bg-white dark:bg-slate-800 rounded-full p-1.5 transition-all duration-300 overflow-hidden border border-slate-200 dark:border-darkBorder shadow-inner group">
                         <div className={`absolute top-1/2 -translate-y-1/2 w-9 h-9 rounded-full shadow-2xl flex items-center justify-center transition-all duration-500 transform ${darkMode ? 'translate-x-12 bg-blue-600' : 'translate-x-0 bg-white'}`}>
                            {darkMode ? <svg className="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M12 3c.132 0 .263 0 .393 0a7.5 7.5 0 0 0 7.92 12.446a9 9 0 1 1-8.313-12.454z"/></svg> : <svg className="w-5 h-5 text-orange-500" fill="currentColor" viewBox="0 0 24 24"><path d="M12 7a5 5 0 1 0 0 10 5 5 0 0 0 0-10zM2 13h2a1 1 0 1 0 0-2H2a1 1 0 1 0 0 2zm18 0h2a1 1 0 1 0 0-2h-2a1 1 0 1 0 0 2zM11 2v2a1 1 0 1 0 2 0V2a1 1 0 1 0-2 0zm0 18v2a1 1 0 1 0 2 0v-2a1 1 0 1 0-2 0z"/></svg>}
                         </div>
                      </button>
                   </div>
               ...
               {settingsView === 'profile' && (
                 <div className="animate-fade-in-up">
                   <div className="flex items-center gap-6 mb-10 bg-slate-50 dark:bg-slate-900/50 p-6 rounded-[32px] border border-slate-100 dark:border-darkBorder">
                     <div className="w-20 h-20 rounded-full bg-blue-600 flex items-center justify-center text-3xl font-black text-white shadow-xl">
                       {userName.charAt(0)}
                     </div>
                     <div>
                   ...
                   <form onSubmit={handleProfileUpdate} className="space-y-4">
                     ...
                     <button type="submit" className="w-full h-14 bg-slate-900 dark:bg-blue-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl active:scale-95 transition-all mt-4">Commit Profile Update</button>
                   </form>
                 </div>
               )}
               ...
               {settingsView === 'report' && (
                 <div className="animate-fade-in-up space-y-6">
                    ...
                    <textarea value={reportText} onChange={(e) => setReportText(e.target.value)} placeholder="Neural feedback or system error details..." className="w-full h-40 bg-slate-50 dark:bg-slate-900/50 border-2 border-slate-100 dark:border-darkBorder rounded-[32px] p-6 text-sm font-bold focus:outline-none focus:border-blue-400 transition-all shadow-inner resize-none" />
                    
                    <div className="flex flex-wrap gap-4">
                      {reportFiles.map((file, i) => (
                        <div key={i} className="px-4 py-2 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                           {file.name}
                    ...
                    {reportStatus && <p className="text-[10px] font-black uppercase tracking-widest text-blue-600 animate-pulse">{reportStatus}</p>}

                    <button onClick={handleSendReport} className="w-full h-16 bg-slate-900 dark:bg-blue-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl active:scale-95 transition-all">Transmit Report</button>
                 </div>
               )}
            </div>
          </div>
            ...
            <div className={`flex flex-col items-center flex-1 w-full max-w-7xl mx-auto ${result ? 'pt-4' : 'justify-center'}`}>
              {!result ? (
                       ...
                       <div className="mt-10 flex flex-col md:flex-row justify-between items-center gap-4">
                          <div className="relative group/director-btn flex-1 md:flex-none">
                            <button 
                                ...
                                className={`flex items-center justify-center gap-2 w-full px-6 h-14 rounded-full font-black text-[10px] uppercase tracking-widest transition-all ${isDirectorMode ? 'bg-blue-600 text-white shadow-lg' : 'bg-white dark:bg-slate-800 text-slate-400 dark:text-slate-500 border border-slate-100 dark:border-darkBorder'}`}
                            >
                          ...
                          <button 
                            ...
                            disabled={loading} 
                            className={`relative w-full md:w-auto h-14 px-12 bg-slate-900 dark:bg-blue-600 text-white rounded-full font-black text-[10px] uppercase tracking-widest hover:bg-blue-600 shadow-xl flex items-center justify-center gap-3 transition-all overflow-hidden ${loading ? 'bg-blue-600' : ''}`}
                          >
                            {loading && (
                ...
                <div className="w-full pb-20 animate-fade-in flex flex-col">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-32 gap-x-12 px-2 items-stretch mb-32">
                    {result.sequence.map((item, index) => {
                      ...
                      return (
                        <div key={item.day} className="relative flex">
                          <div className={`group relative w-full rounded-[44px] md:rounded-[56px] p-10 pt-32 bg-white dark:bg-darkCard border-2 transition-all duration-700 flex flex-col items-center overflow-visible hover:shadow-heavy ${stage.border} h-full`}>
                            <div className={`absolute inset-0 transition-opacity duration-700 rounded-[44px] md:rounded-[56px] ${stage.tint} ${stage.hoverTint}`}></div>
                            <div className="absolute top-8 left-10 right-10 flex items-center justify-between z-20">
                               <span className={`${stage.textColor} font-black text-[10px] tracking-[0.4em] uppercase`}>{stage.label}</span>
                               <div className={`px-5 h-12 bg-white dark:bg-slate-800 border-[3px] ${stage.borderColor} rounded-[22px] flex items-center justify-center font-black ${stage.textColor} text-sm`}>Day {item.day}</div>
                            </div>
                            <div className="relative z-10 w-full flex flex-col items-center h-full">
                              <h4 className="font-black text-slate-900 dark:text-white text-2xl md:text-3xl mb-8 text-center leading-tight">"{item.hook}"</h4>
                              <p className="text-base md:text-lg font-medium text-slate-500 dark:text-slate-400 mb-10 text-center leading-relaxed italic opacity-90">{item.value}</p>
                              <div className="w-full pt-10 border-t border-slate-100 dark:border-slate-800 flex flex-col items-center mb-10 mt-auto">
                            ...
                            <div className="absolute -bottom-20 left-1/2 -translate-x-1/2 flex flex-col items-center z-30">
                              ...
                              <div className="group/info relative">
                                ...
                                <div className="absolute top-full left-1/2 -translate-x-1/2 mt-4 w-64 p-6 bg-slate-900 text-white rounded-[24px] shadow-2xl opacity-0 invisible group-hover/info:opacity-100 group-hover/info:visible transition-all duration-300 z-50 border border-white/10">
                                  <p className="text-[9px] font-black text-blue-400 uppercase tracking-widest mb-2">Proceed Criteria</p>
                       ...
                       <div className="mt-8 flex flex-col md:flex-row justify-between items-center gap-4">
                          ...
                          <div className="flex flex-col md:flex-row gap-4 w-full md:w-auto">
                            ...
                            <button 
                              ...
                              disabled={loading} 
                              className={`relative w-full md:w-auto h-14 px-12 bg-slate-900 dark:bg-blue-600 text-white rounded-full font-black text-[10px] uppercase tracking-widest hover:bg-blue-600 shadow-xl flex items-center justify-center gap-3 transition-all overflow-hidden ${(loading && loadingSource === 'update') ? 'bg-blue-600' : ''}`}
                            >
          ...
          {activeTab === 'history' && (
            <div className="max-w-7xl mx-auto w-full px-4 md:px-12 pb-20">
               ...
               {history.length === 0 ? (
                 ...
                 <div key={idx} className="bg-white dark:bg-darkCard p-8 md:p-10 rounded-[32px] md:rounded-[44px] border border-slate-100 dark:border-darkBorder shadow-premium flex flex-col md:flex-row items-center justify-between mb-6 gap-6 transition-all hover:scale-[1.01]">
                   <div className="text-center md:text-left">
                      <h4 className="text-2xl font-black uppercase text-slate-900 dark:text-white mb-1">{item.result.productName}</h4>
                   ...
                   <div className="flex items-center gap-4 w-full md:w-auto">
                    ...
                    <button 
                      ...
                      className="p-4 rounded-2xl border border-slate-100 dark:border-darkBorder text-slate-300 hover:text-red-500 transition-colors"
                    >
