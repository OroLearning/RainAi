  ...
  return (
    <div 
      ...
      className={`reveal relative group h-64 rounded-[32px] p-8 flex flex-col justify-end transition-all duration-500 hover:-translate-y-2
        ${isDark 
          ? 'bg-[#21337e] dark:bg-blue-900 text-white shadow-2xl shadow-blue-900/20 dark:shadow-[0_0_50px_-10px_rgba(59,130,246,0.4)]' 
    ...
    >
      <div className={`absolute top-0 right-0 w-24 h-24 rounded-bl-[40px] rounded-tr-[32px] flex items-center justify-center
        ${isDark ? 'bg-white/10 backdrop-blur-md' : 'bg-[#f4f7ff] dark:bg-slate-800'}`}>
        <Icon className={`w-10 h-10 ${isDark ? 'text-white' : 'text-[#3b82f6]'}`} />
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-20 items-center">
        <div className="reveal">
          <div className="flex items-center gap-2 mb-6">
            ...
            <span className="text-blue-600 dark:text-blue-400 font-extrabold tracking-widest text-xs uppercase">Core Engine</span>
          </div>
          <h2 className="text-6xl font-black text-[#1e293b] dark:text-white mb-8 leading-[1.1]">
            The Science <br />
            <span className="text-blue-600">Of Selling.</span>
          </h2>
          <p className="text-xl text-slate-500 dark:text-slate-400 leading-relaxed max-w-lg mb-12">
            StoryLaunch isn't just a text generator. It's a psychological framework designed to move your audience from passive observers to active buyers in exactly 14 days.
          </p>
        </div>
